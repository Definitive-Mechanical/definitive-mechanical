...
export default function ResidentialPlumbing() {
  return (
    <>
      ...
      {/* SECTION 1: HERO */}
      <section style={{ minHeight: '55vh', background: 'linear-gradient(135deg, #003060 0%, #186090 100%)', display: 'flex', alignItems: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {SERVICE_CARDS.map(({ Icon, title, items }) => (
              <div key={title} style={{ background: 'white', borderTop: '4px solid #009EC6', borderRadius: '6px', padding: '24px', boxShadow: '0 2px 8px rgba(0,48,96,0.06)' }}>
                <Icon size={32} color="#009EC6" style={{ marginBottom: '12px' }} />
                <p style={{ fontFamily: 'Barlow, Arial, sans-serif', fontWeight: 600, color: '#003060', fontSize: '16px', marginBottom: '12px' }}>{title}</p>
                <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                  {items.map((item) => (
                    <li key={item} style={{ fontFamily: 'Barlow, Arial, sans-serif', color: '#484848', fontSize: '13px', lineHeight: 1.6, paddingBottom: '4px' }}>• {item}</li>
                  ))}
                </ul>
              </div>
            ))}
      ...
      <section style={{ background: '#003060', padding: '80px 0' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p style={{ fontFamily: 'Barlow, Arial, sans-serif', color: 'rgba(255,255,255,0.8)', fontSize: '16px', lineHeight: 1.7, maxWidth: '700px', margin: '0 auto 40px', textAlign: 'center' }}>
            ...
            {RISK_CARDS.map(({ Icon, title, body }) => (
              <div key={title} style={{ background: 'white', borderRadius: '6px', padding: '24px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                  ...
                  <p style={{ fontFamily: 'Barlow, Arial, sans-serif', fontWeight: 600, color: '#003060', fontSize: '15px' }}>{title}</p>
                </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', justifyContent: 'center', marginTop: '32px' }}>
            {["MD #96958", "DC #PGM1002236", "VA #2710064209", "DE #PL-0012866"].map((badge) => (
              <span key={badge} style={{ background: 'white', color: '#003060', fontFamily: 'Barlow Condensed, Arial, sans-serif', fontWeight: 700, fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.08em', padding: '6px 14px', borderRadius: '3px' }}>{badge}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <section style={{ background: '#F0F0F0', padding: '64px 0' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <div>
              <h2 style={{ fontFamily: 'Playfair Display, Georgia, serif', fontWeight: 700, color: '#003060', fontSize: 'clamp(20px, 2.5vw, 26px)', marginBottom: '8px' }}>How does Definitive Mechanical's pricing work for residential clients?</h2>
              <p style={{ fontFamily: 'Barlow, Arial, sans-serif', fontWeight: 600, color: '#009EC6', fontSize: '15px', marginBottom: '16px' }}>Upfront pricing in writing — before any work begins.</p>
              ...
              <div className="space-y-2">
                {["Written pricing before we touch a pipe", "No commissioned salespeople", "Same rate day or night — no overtime surcharge"].map((item) => (
                  <div key={item} style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    ...
                    <span style={{ fontFamily: 'Barlow, Arial, sans-serif', fontWeight: 600, color: '#003060', fontSize: '14px' }}>{item}</span>
                  </div>
                ))}
              </div>
            </div>
            <div>
              <img src="https://images.unsplash.com/photo-1581578731548-c64695cc6952?w=600&q=80" alt="Upfront pricing residential plumbing Maryland DC Virginia" style={{ width: '100%', borderRadius: '6px', boxShadow: '0 4px 20px rgba(0,48,96,0.12)', objectFit: 'cover', aspectRatio: '3/2' }} />
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: 'white', padding: '48px 0' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div style={{ background: '#C0D8F0', borderRadius: '8px', padding: '28px', maxWidth: '700px', margin: '0 auto', display: 'flex', alignItems: 'center', gap: '24px', flexWrap: 'wrap' }}>
            <CreditCard size={48} color="#003060" style={{ flexShrink: 0 }} />
            <div style={{ flex: 1 }}>
              <p style={{ fontFamily: 'Barlow, Arial, sans-serif', color: '#484848', fontSize: '15px', lineHeight: 1.7, marginBottom: '16px' }}>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
            {[
              ...
              <Link key={item.label} href={item.href} style={{ background: 'white', border: '1px solid #E8EFF5', borderRadius: '4px', padding: '14px', display: 'flex', alignItems: 'center', gap: '10px', textDecoration: 'none' }}>
                <item.icon size={18} color="#009EC6" style={{ flexShrink: 0 }} />
                <span style={{ fontFamily: 'Barlow, Arial, sans-serif', fontWeight: 600, color: '#003060', fontSize: '12px' }}>{item.label}</span>
              </Link>
            ))}
          </div>
        </div>
